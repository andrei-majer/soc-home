...
- **🛜 OpenWrt Edge Router** — `.1` managed via `raw` + `scp` (no Python on target); **AdGuard Home** blocks 47k+ ad/tracker/malware domains, **Unbound** provides full DNSSEC recursion (no third-party DNS forwarding), DoT/DoQ/DoH endpoints exposed, DNS bypass firewalled at WAN; **BanIP** holds 47k+ active entries from **Hagezi**, Spamhaus DROP, DShield, threat, threatview — independent reputation feeds, with SIEM-detected offenders added in via fail2ban
...
|---|---|---|---|---|
| **suricata-120** | 192.168.1.120 | Debian 12 | `common`, `suricata` (incl. Zeek, Snort 3, Filebeat, Grafana, Loki, Promtail, EveBox, Arkime, Velociraptor, fail2ban, ntfy) | `local` (control node) |
...
`ansible-playbook playbooks/site.yml` reports **0 changed, 0 failed** on all 6 hosts when the lab is healthy. Any `changed` items are drift since the last convergence and should be investigated.

`ansible-lint` passes the **`production` profile** (strictest) across all Ansible files (34+ as of last verification).
