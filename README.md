...
- **🤖 SOAR-Lite Containment** — `soc-contain` on `.120`: a stdlib-Python containment receiver (`192.168.1.120:8765`) fed by Wazuh active-response. **Tiered** (T0 auto-contain / T1 human-approval), **guardrailed** (never acts on the SOC infra hosts), **reversible** (channel/DNS/router bans auto-release at TTL via a reaper; host-isolation stays manual), with a **dry-run → armed lifecycle** and an instant file-based kill-switch. Actuators: router ban-set, DNS sinkhole, Velociraptor collection, **Velociraptor host-firewall quarantine (host-level IPsec L2 lockdown — stops same-subnet lateral movement a gateway block can't reach on the flat LAN; composed with router-isolate for a compromised managed host)**, Tailscale quarantine, ntfy. **Full service source** (stdlib-only Python, TDD): **[`andrei-majer/soc-contain`](https://github.com/andrei-majer/soc-contain)**
