...
- **🔔 Telegram Alerting** — every Wazuh integration alert reaches a private Telegram bot with per-(rule, source) hourly dedup at the integration script. Migrated off self-hosted ntfy on 2026-06-29; the ntfy vars and `custom-ntfy` script are retained for rollback but no longer wired into `ossec.conf`
...
| **Alerting** | Telegram bot · `custom-telegram` Wazuh integration with per-(rule, source) hourly dedup · router-side `soc-watchdog` for out-of-VM alerting. (Migrated off self-hosted [ntfy](https://ntfy.sh/) 2026-06-29; `custom-ntfy` retained for rollback, not wired into `ossec.conf`) |
